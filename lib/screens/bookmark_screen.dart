...
Future<Map<String, dynamic>?> _fetchWeatherForDate(
  ...
  final future = _weatherFutureCache.putIfAbsent(cacheKey, () async {
    ...
    try {
      ...
      final List tmax = (data['daily']?['temperature_2m_max'] as List?) ?? const [];
      final List tmin = (data['daily']?['temperature_2m_min'] as List?) ?? const [];
      final List prcpProb = (data['daily']?['precipitation_probability_mean'] as List?) ?? const [];
  ...
  _weatherFutureCache.remove(cacheKey);
  return res;
}
...
class _WeatherPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = '${temp?.toStringAsFixed(1) ?? '-'}℃${rain != null ? ' · ${rain}%' : ''}';
    return Container(
...
class _AvailPill extends StatelessWidget {
  ...
  final int avail;
  final int total;
  const _AvailPill({required this.isAvailable, required this.avail, required this.total});
  @override
  Widget build(BuildContext context) {
    final color = isAvailable ? const Color(0xFF16A34A) : const Color(0xFFDC2626);
    final bg = isAvailable ? const Color(0xFFE9F7EE) : const Color(0xFFFDECEC);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
          ...
          itemBuilder: (_, i) {
            ...
            return FutureBuilder<Availability>(
              ...
              builder: (context, snap1) {
                ...
                final lat = _toDouble(matching['mapY']);
                final lng = _toDouble(matching['mapX']);
                ...
                return FutureBuilder<Map<String, dynamic>?>(
                  ...
                  builder: (context, wsnap) {
                    ...
                    if (waiting) return _BookmarkSkeletonCard();
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          if (contentId.isNotEmpty)
                                            _LiveAverageRatingBadge(contentId: contentId),
                                          IconButton(
                                      ...
                                      Row(
                                        children: [
                                          if (weather != null)
                                            _WeatherPill(
                                              wmo: (weather['wmo'] as num?)?.toInt(),
                                              temp: (weather['temp'] as num?)?.toDouble(),
                                              rain: weather['chanceOfRain'] as int?,
                                            ),
                                          const Spacer(),
                                          _AvailPill(isAvailable: isAvail, avail: available, total: total),
                                        ],
                                      ),
                                    ],
                                  ),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Container(height: 16, width: 44, color: Colors.white),
                      ],
                    ),
...
class _ImageFallback extends StatelessWidget {
  final double size;
  const _ImageFallback({this.size = 80});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Icon(Icons.park, color: Colors.teal, size: 36),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        if (cnt == 0) return const SizedBox.shrink();
        final avgText = (sum / cnt).toStringAsFixed(1);
        return Text(avgText, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 12));
      },
    );
  }
}
