  ...
  void _openFilterSheet() {
    final regions =
        _allCamps
          ...
          ..sort();
    final types =
        _allCamps.map((c) => c['type'] as String).toSet().toList()..sort();
    final duties =
        _allCamps
            .expand(
              (c) => (c['inDuty'] as String? ?? '')
                  .split(',')
            ...
            child: DraggableScrollableSheet(
              ...
              builder:
                  (_, controller) => StatefulBuilder(
                    builder:
                        (ctx2, setModalState) => SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildSection(
                                ...
                                onSelected:
                                    (v) =>
                                        setModalState(() => _appliedRegion = v),
                              ),
                              _buildSection(
                                title: '캠핑장 유형',
                                options: types,
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<List<Map<String, dynamic>>>(
              stream: _repo.campgroundsStream(),
              builder: (ctx, campSnap) {
                if (!campSnap.hasData) {
                  return const Center(child: CircularProgressIndicator());
                }
                // 최신 캠핑장 목록 저장(필터 시트용)
                _allCamps = campSnap.data!;

                return StreamBuilder<Map<String, Map<String, dynamic>>>(
                  stream: _repo.availabilityStream(),
                  builder: (ctx2, availSnap) {
                    if (!availSnap.hasData) {
                      return const Center(child: CircularProgressIndicator());
                    }
                    final availabilityMap = availSnap.data!;
                        ...
                        _allCamps.where((c) {
                          ...
                          if (_appliedDuty != null) {
                            final duties = (c['inDuty'] as String? ?? '')
                                .split(',')
                                .where((s) => s.isNotEmpty);
                            if (!duties.contains(_appliedDuty)) return false;
                          }
                          if (_appliedEnv != null &&
